...
uint8_t playDouble[8] = {0x80, 0x0C, 0x0A, 0x0A, 0x0A, 0x0C, 0x00, 0x00};
uint8_t playTriple[8] = {0x80, 0x0E, 0x04, 0x04, 0x04, 0x04, 0x00, 0x00};
uint8_t playHomeRun[8] = {0x14, 0x1C, 0x14, 0x00, 0x06, 0x05, 0x06, 0x05};
uint8_t playSacrificeHit[8] = {0x1C, 0x10, 0x1C, 0x04, 0x1C, 0x05, 0x07, 0x05};
uint8_t playWalk[8] = {0x18, 0x14, 0x1C, 0x16, 0x1D, 0x07, 0x05, 0x06};
uint8_t playStrikeout[8] = {0x80, 0x0A, 0x0A, 0x0C, 0x0A, 0x0A, 0x00, 0x00};
uint8_t playCalledStrikeout[8] = {0x80, 0x0A, 0x0A, 0x06, 0x0A, 0x0A, 0x00, 0x00};
uint8_t playBalk[8] = {0x18, 0x14, 0x1C, 0x15, 0x1D, 0x06, 0x05, 0x05};
uint8_t playFieldersChoice[8] = {0x1C, 0x10, 0x1C, 0x10, 0x17, 0x04, 0x04, 0x07};
uint8_t playHitByPitch[8] = {0x14, 0x1C, 0x14, 0x00, 0x07, 0x05, 0x07, 0x04};
uint8_t playWildPitch[8] = {0x11, 0x15, 0x1B, 0x00, 0x07, 0x05, 0x07, 0x04};
uint8_t playPassedBall[8] = {0x1C, 0x14, 0x1C, 0x12, 0x05, 0x07, 0x05, 0x06};
uint8_t playStolenBase[8] = {0x1C, 0x10, 0x1C, 0x06, 0x1D, 0x07, 0x05, 0x06};
uint8_t playDoublePlay[8] = {0x18, 0x14, 0x14, 0x18, 0x07, 0x05, 0x07, 0x04};
uint8_t playError[8] = {0x80, 0x8E, 0x88, 0x8C, 0x88, 0x8E, 0x00, 0x00};
uint8_t playSacrificeFly[8] = {0x1C, 0x10, 0x1C, 0x07, 0x1C, 0x07, 0x04, 0x04};
uint8_t playIntentionalWalk[8] = {0x10, 0x10, 0x10, 0x06, 0x05, 0x07, 0x05, 0x06};
uint8_t playFoul[8] = {0x80, 0x8E, 0x88, 0x8E, 0x88, 0x88, 0x00, 0x00};
uint8_t playForceOut[8] = {0x1C, 0x10, 0x1C, 0x10, 0x17, 0x05, 0x05, 0x07};
uint8_t playLineDrive[8] = {0x80, 0x88, 0x88, 0x88, 0x88, 0x8E, 0x00, 0x00};
uint8_t playBunt[8] = {0x80, 0x0C, 0x0A, 0x0E, 0x0A, 0x0C, 0x00, 0x00};
uint8_t playUnassisted[8] = {0x80, 0x0A, 0x0A, 0x0A, 0x0A, 0x06, 0x00, 0x00};

uint8_t play1[8] = {0x80, 0x04, 0x04, 0x04, 0x04, 0x04, 0x00, 0x00};
uint8_t play2[8] = {0x80, 0x8E, 0x82, 0x8E, 0x88, 0x8E, 0x00, 0x00};
uint8_t play3[8] = {0x80, 0x0E, 0x02, 0x06, 0x02, 0x0E, 0x00, 0x00};
uint8_t play4[8] = {0x80, 0x0A, 0x0A, 0x0E, 0x02, 0x02, 0x00, 0x00};
uint8_t play5[8] = {0x80, 0x8E, 0x88, 0x8C, 0x82, 0x8C, 0x00, 0x00};
uint8_t play6[8] = {0x80, 0x8E, 0x88, 0x8E, 0x8A, 0x8C, 0x00, 0x00};
uint8_t play7[8] = {0x80, 0x0E, 0x02, 0x02, 0x02, 0x02, 0x00, 0x00};
...
int bases(String command) {
    ...
    // Draw character on LCD
    if (command.indexOf("123") >= 0) {
      printBases(basesOccupied123);
    } else if (command.indexOf("103") >= 0) {
      printBases(basesOccupied13);
    } else if (command.indexOf("120") >= 0) {
      printBases(basesOccupied12);
    } else if (command.indexOf("023") >= 0) {
      printBases(basesOccupied23);
    } else if (command.indexOf("103") >= 0) {
      printBases(basesOccupied13);
    } else if (command.indexOf("003") >= 0) {
      printBases(basesOccupied3);
    } else if (command.indexOf("020") >= 0) {
      printBases(basesOccupied2);
    } else if (command.indexOf("100") >= 0) {
      printBases(basesOccupied1);
    } else {
      printBases(basesOccupied0);
    }
...
void printBases(uint8_t charmap[]) {
    lcd.createChar(CHAR_BASES_OCCUPIED, charmap);
    lcd.setCursor(2, 10);
    lcd.printCustomChar(CHAR_BASES_OCCUPIED);
}
